...
http.createServer(function (request1, response) {
    ...
    //create a SdsStream
    console.log("Creating a SdsStream")
    ...
    // Interpolation
    var propertyOverride = new sdsObjs.SdsStreamPropertyOverride({ "SdsTypePropertyId": "Radians",  "InterpolationModeOverride": sdsObjs.sdsStreamMode.Discrete });
    ...
    var dumpViewEvent = getRangeViewEvents.then(
        function (res) {
            var obj = JSON.parse(res);
            console.log("\nSpecifying a view with a SdsType of the same shape returns values that are automatically mapped to the target SdsType's properties:");
            obj.forEach(function (elem) {
